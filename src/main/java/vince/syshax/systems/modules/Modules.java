...
import vince.syshax.events.meteor.ActiveModulesChangedEvent;
import vince.syshax.events.meteor.KeyEvent;
import vince.syshax.events.meteor.ModuleBindChangedEvent;
...
import  vince.syshax.systems.modules.combat.*;
import  vince.syshax.systems.modules.misc.*;
import vince.syshax.systems.modules.misc.swarm.Swarm;
import  vince.syshax.systems.modules.movement.*;
import vince.syshax.systems.modules.movement.elytrafly.ElytraFly;
import vince.syshax.systems.modules.movement.speed.Speed;
import  vince.syshax.systems.modules.player.*;
import  vince.syshax.systems.modules.render.*;
import vince.syshax.systems.modules.render.marker.Marker;
import vince.syshax.systems.modules.render.search.Search;
import vince.syshax.systems.modules.world.*;
...
import vince.syshax.utils.misc.ValueComparableMap;
import vince.syshax.utils.misc.input.Input;
...
public class Modules extends System<Modules> {
    ...
    void addActive(Module module) {
        synchronized (active) {
            if (!active.contains(module)) {
                active.add(module);
                SYSHax.EVENT_BUS.post(ActiveModulesChangedEvent.get());
            }
        }
    }
    ...
    private boolean onBinding(boolean isKey, int value) {
        ...
        SYSHax.EVENT_BUS.post(ModuleBindChangedEvent.get(moduleToBind));
        moduleToBind = null;
        ...
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    SYSHax.EVENT_BUS.subscribe(module);
                    module.onActivate();
                }
            }
        }
    ...
    private void onGameLeft(GameLeftEvent event) {
        synchronized (active) {
            for (Module module : modules) {
                if (module.isActive() && !module.runInMainMenu) {
                    SYSHax.EVENT_BUS.unsubscribe(module);
                    module.onDeactivate();
                }
            }
        }
