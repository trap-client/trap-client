...
import vince.syshax.utils.player.EChestMemory;
import vince.syshax.utils.render.PeekScreen;
import vince.syshax.utils.render.color.Color;
import vince.syshax.utils.world.BlockEntityIterator;
import vince.syshax.utils.world.ChunkIterator;
...
public class Utils {
    ...
    public static void init() {
        SYSHax.EVENT_BUS.subscribe(Utils.class);
    }

    @EventHandler
    private static void onTick(TickEvent.Post event) {
        if (screenToOpen != null && SYSHax.mc.currentScreen == null) {
            SYSHax.mc.setScreen(screenToOpen);
            screenToOpen = null;
        }
    }

    public static double getPlayerSpeed() {
        if (SYSHax.mc.player == null) return 0;

        double tX = Math.abs(SYSHax.mc.player.getX() - SYSHax.mc.player.prevX);
        double tZ = Math.abs(SYSHax.mc.player.getZ() - SYSHax.mc.player.prevZ);
        double length = Math.sqrt(tX * tX + tZ * tZ);

        vince.syshax.systems.modules.world.Timer timer = Modules.get().get(vince.syshax.systems.modules.world.Timer.class);
    ...
    public static String getWorldTime() {
        if (SYSHax.mc.world == null) return "00:00";

        int ticks = (int) (SYSHax.mc.world.getTimeOfDay() % 24000);
        ticks += 6000;
    ...
    public static int getRenderDistance() {
        return Math.max(SYSHax.mc.options.getViewDistance().getValue(), ((ClientPlayNetworkHandlerAccessor) SYSHax.mc.getNetworkHandler()).getChunkLoadDistance());
    }

    public static int getWindowWidth() {
        return SYSHax.mc.getWindow().getFramebufferWidth();
    }

    public static int getWindowHeight() {
        return SYSHax.mc.getWindow().getFramebufferHeight();
    }

    public static void unscaledProjection() {
        RenderSystem.setProjectionMatrix(Matrix4f.projectionMatrix(0, SYSHax.mc.getWindow().getFramebufferWidth(), 0, SYSHax.mc.getWindow().getFramebufferHeight(), 1000, 3000));
        rendering3D = false;
    }

    public static void scaledProjection() {
        RenderSystem.setProjectionMatrix(Matrix4f.projectionMatrix(0, (float) (SYSHax.mc.getWindow().getFramebufferWidth() / SYSHax.mc.getWindow().getScaleFactor()), 0, (float) (SYSHax.mc.getWindow().getFramebufferHeight() / SYSHax.mc.getWindow().getScaleFactor()), 1000, 3000));
        rendering3D = true;
    }
    ...
    public static boolean openContainer(ItemStack itemStack, ItemStack[] contents, boolean pause) {
        if (hasItems(itemStack) || itemStack.getItem() == Items.ENDER_CHEST) {
            ...
            else SYSHax.mc.setScreen(new PeekScreen(itemStack, contents));
            return true;
        }
    ...
    public static String getWorldName() {
        // Singleplayer
        if (SYSHax.mc.isInSingleplayer()) {
            if (SYSHax.mc.world == null) return "";

            File folder = ((MinecraftServerAccessor) SYSHax.mc.getServer()).getSession().getWorldDirectory(SYSHax.mc.world.getRegistryKey()).toFile();
            if (folder.toPath().relativize(SYSHax.mc.runDirectory.toPath()).getNameCount() != 2) {
        ...
        // Multiplayer
        if (SYSHax.mc.getCurrentServerEntry() != null) {
            String name = SYSHax.mc.isConnectedToRealms() ? "realms" : SYSHax.mc.getCurrentServerEntry().address;
    ...
    public static boolean canUpdate() {
        return SYSHax.mc != null && SYSHax.mc.world != null && SYSHax.mc.player != null;
    }

    public static boolean canOpenGui() {
        if (canUpdate()) return SYSHax.mc.currentScreen == null;

        return SYSHax.mc.currentScreen instanceof TitleScreen || SYSHax.mc.currentScreen instanceof MultiplayerScreen || SYSHax.mc.currentScreen instanceof SelectWorldScreen;
    }
    ...
    public static void leftClick() {
        SYSHax.mc.options.attackKey.setPressed(true);
        ((MinecraftClientAccessor) SYSHax.mc).leftClick();
        SYSHax.mc.options.attackKey.setPressed(false);
    }

    public static void rightClick() {
        ((IMinecraftClient) SYSHax.mc).rightClick();
    }
    ...
    public static boolean isLoading() {
        ResourceReloadLogger.ReloadState state = ((ResourceReloadLoggerAccessor) ((MinecraftClientAccessor) SYSHax.mc).getResourceReloadLogger()).getReloadState();
