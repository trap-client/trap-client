...
public class BlockIterator {
    ...
    public static void init() {
        SYSHax.EVENT_BUS.subscribe(BlockIterator.class);
    }
    ...
    private static void onTick(TickEvent.Pre event) {
        ...
        int px = SYSHax.mc.player.getBlockX();
        int py = SYSHax.mc.player.getBlockY();
        int pz = SYSHax.mc.player.getBlockZ();

        for (int x = px - hRadius; x <= px + hRadius; x++) {
            for (int z = pz - hRadius; z <= pz + hRadius; z++) {
                for (int y = Math.max(SYSHax.mc.world.getBottomY(), py - vRadius); y <= py + vRadius; y++) {
                    if (y > SYSHax.mc.world.getTopY()) break;

                    blockPos.set(x, y, z);
                    BlockState blockState = SYSHax.mc.world.getBlockState(blockPos);
