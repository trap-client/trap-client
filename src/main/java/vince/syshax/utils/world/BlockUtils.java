...
public class BlockUtils {
    ...
    public static void init() {
        SYSHax.EVENT_BUS.subscribe(BlockUtils.class);
    }
    ...
    public static boolean place(BlockPos blockPos, FindItemResult findItemResult, boolean rotate, int rotationPriority, boolean swingHand, boolean checkEntities, boolean swapBack) {
        if (findItemResult.isOffhand()) {
            return place(blockPos, Hand.OFF_HAND, SYSHax.mc.player.getInventory().selectedSlot, rotate, rotationPriority, swingHand, checkEntities, swapBack);
    ...
    private static void place(BlockHitResult blockHitResult, Hand hand, boolean swing) {
        boolean wasSneaking = SYSHax.mc.player.input.sneaking;
        SYSHax.mc.player.input.sneaking = false;

        ActionResult result = SYSHax.mc.interactionManager.interactBlock(SYSHax.mc.player, hand, blockHitResult);

        if (result.shouldSwingHand()) {
            if (swing) SYSHax.mc.player.swingHand(hand);
            else SYSHax.mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(hand));
        }

        SYSHax.mc.player.input.sneaking = wasSneaking;
    }
    ...
    public static boolean canPlace(BlockPos blockPos, boolean checkEntities) {
        ...
        if (!SYSHax.mc.world.getBlockState(blockPos).getMaterial().isReplaceable()) return false;

        // Check if intersects entities
        return !checkEntities || SYSHax.mc.world.canPlace(SYSHax.mc.world.getBlockState(blockPos), blockPos, ShapeContext.absent());
    }
    ...
    public static Direction getPlaceSide(BlockPos blockPos) {
        for (Direction side : Direction.values()) {
            ...
            BlockState state = SYSHax.mc.world.getBlockState(neighbor);
    ...
    private static void onTickPost(TickEvent.Post event) {
        if (!breakingThisTick && breaking) {
            breaking = false;
            if (SYSHax.mc.interactionManager != null) SYSHax.mc.interactionManager.cancelBlockBreaking();
        }
    }

    /** Needs to be used in {@link TickEvent.Pre} */
    public static boolean breakBlock(BlockPos blockPos, boolean swing) {
        if (!canBreak(blockPos, SYSHax.mc.world.getBlockState(blockPos))) return false;
        ...
        if (SYSHax.mc.interactionManager.isBreakingBlock()) SYSHax.mc.interactionManager.updateBlockBreakingProgress(pos, Direction.UP);
        else SYSHax.mc.interactionManager.attackBlock(pos, Direction.UP);

        if (swing) SYSHax.mc.player.swingHand(Hand.MAIN_HAND);
        else SYSHax.mc.getNetworkHandler().sendPacket(new HandSwingC2SPacket(Hand.MAIN_HAND));
    ...
    public static boolean canBreak(BlockPos blockPos, BlockState state) {
        if (!SYSHax.mc.player.isCreative() && state.getHardness(SYSHax.mc.world, blockPos) < 0) return false;
        return state.getOutlineShape(SYSHax.mc.world, blockPos) != VoxelShapes.empty();
    }
    public static boolean canBreak(BlockPos blockPos) {
        return canBreak(blockPos, SYSHax.mc.world.getBlockState(blockPos));
    }

    public static boolean canInstaBreak(BlockPos blockPos, BlockState state) {
        return SYSHax.mc.player.isCreative() || state.calcBlockBreakingDelta(SYSHax.mc.player, SYSHax.mc.world, blockPos) >= 1;
    }
    public static boolean canInstaBreak(BlockPos blockPos) {
        return canInstaBreak(blockPos, SYSHax.mc.world.getBlockState(blockPos));
    }
    ...
    public static MobSpawn isValidMobSpawn(BlockPos blockPos, boolean newMobSpawnLightLevel) {
        ...
        if (!(SYSHax.mc.world.getBlockState(blockPos).getBlock() instanceof AirBlock) ||
            SYSHax.mc.world.getBlockState(blockPos.down()).getBlock() == Blocks.BEDROCK) return MobSpawn.Never;

        if (!topSurface(SYSHax.mc.world.getBlockState(blockPos.down()))) {
            if (SYSHax.mc.world.getBlockState(blockPos.down()).getCollisionShape(SYSHax.mc.world, blockPos.down()) != VoxelShapes.fullCube()) return MobSpawn.Never;
            if (SYSHax.mc.world.getBlockState(blockPos.down()).isTranslucent(SYSHax.mc.world, blockPos.down())) return MobSpawn.Never;
        }

        if (SYSHax.mc.world.getLightLevel(blockPos, 0) <= spawnLightLimit) return MobSpawn.Potential;
        else if (SYSHax.mc.world.getLightLevel(LightType.BLOCK, blockPos) <= spawnLightLimit) return MobSpawn.Always;
    ...
    public static boolean isExposed(BlockPos blockPos) {
        for (Direction direction : Direction.values()) {
            if (!SYSHax.mc.world.getBlockState(EXPOSED_POS.get().set(blockPos, direction)).isOpaque()) return true;
        }
