...
public class NbtUtils {
    ...
    public static boolean toClipboard(String name, NbtCompound nbtCompound) {
        String preClipboard = SYSHax.mc.keyboard.getClipboard();
        try {
            ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
            NbtIo.writeCompressed(nbtCompound, byteArrayOutputStream);
            SYSHax.mc.keyboard.setClipboard(Base64.getEncoder().encodeToString(byteArrayOutputStream.toByteArray()));
            return true;
        } catch (Exception e) {
            SYSHax.LOG.error(String.format("Error copying %s NBT to clipboard!", name));
            ...
            SYSHax.mc.keyboard.setClipboard(preClipboard);
            return false;
        }
    }
    ...
    public static NbtCompound fromClipboard(NbtCompound schema) {
        try {
            byte[] data = Base64.getDecoder().decode(SYSHax.mc.keyboard.getClipboard().trim());
            ...
            SYSHax.LOG.error("Invalid NBT data pasted!");
