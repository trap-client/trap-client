...
import vince.syshax.mixin.ClientPlayerInteractionManagerAccessor;
import vince.syshax.mixin.MinecraftClientAccessor;
...
import vince.syshax.utils.world.TickRate;
...
public class MeteorStarscript {
    ...
    public static void init() {
        ...
        // General
        ss.set("version", SYSHax.VERSION != null ? (SYSHax.DEV_BUILD.isEmpty() ? SYSHax.VERSION.toString() : SYSHax.VERSION + " " + SYSHax.DEV_BUILD) : "");
        ...
        ss.set("camera", new ValueMap()
            .set("pos", new ValueMap()
                ...
                .set("x", () -> Value.number(SYSHax.mc.gameRenderer.getCamera().getPos().x))
                .set("y", () -> Value.number(SYSHax.mc.gameRenderer.getCamera().getPos().y))
                .set("z", () -> Value.number(SYSHax.mc.gameRenderer.getCamera().getPos().z))
            )
        ...
        ss.set("player", new ValueMap()
            .set("_toString", () -> Value.string(SYSHax.mc.getSession().getUsername()))
            .set("health", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.getHealth() : 0))
            .set("hunger", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.getHungerManager().getFoodLevel() : 0))
            .set("speed", () -> Value.number(Utils.getPlayerSpeed()))

            .set("breaking_progress", () -> Value.number(SYSHax.mc.interactionManager != null ? ((ClientPlayerInteractionManagerAccessor) SYSHax.mc.interactionManager).getBreakingProgress() : 0))
            ...
            .set("pos", new ValueMap()
                ...
                .set("x", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.getX() : 0))
                .set("y", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.getY() : 0))
                .set("z", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.getZ() : 0))
            )
            .set("opposite_dim_pos", new ValueMap()
            ...
            .set("hand", () -> SYSHax.mc.player != null ? wrap(SYSHax.mc.player.getMainHandStack()) : Value.null_())
            .set("offhand", () -> SYSHax.mc.player != null ? wrap(SYSHax.mc.player.getOffHandStack()) : Value.null_())
            ...
            .set("xp", new ValueMap()
                .set("level", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.experienceLevel : 0))
                .set("progress", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.experienceProgress : 0))
                .set("total", () -> Value.number(SYSHax.mc.player != null ? SYSHax.mc.player.totalExperience : 0))
            )
        ...
        ss.set("server", new ValueMap()
            ...
            .set("player_count", () -> Value.number(SYSHax.mc.getNetworkHandler() != null ? SYSHax.mc.getNetworkHandler().getPlayerList().size() : 0))
            .set("difficulty", () -> Value.string(SYSHax.mc.world != null ? SYSHax.mc.world.getDifficulty().getName() : ""))
        );
    }
    ...
    private static Value hasPotionEffect(Starscript ss, int argCount) {
        ...
        if (SYSHax.mc.player == null) return Value.bool(false);
        ...
        StatusEffectInstance effectInstance = SYSHax.mc.player.getStatusEffect(effect);
    ...
    private static Value getPotionEffect(Starscript ss, int argCount) {
        ...
        if (SYSHax.mc.player == null) return Value.null_();
    ...
    private static Value getStat(Starscript ss, int argCount) {
        ...
        if (SYSHax.mc.player == null) return Value.number(0);

        long time = System.currentTimeMillis();
        if ((time - lastRequestedStatsTime) / 1000.0 >= 1 && SYSHax.mc.getNetworkHandler() != null) {
            SYSHax.mc.getNetworkHandler().sendPacket(new ClientStatusC2SPacket(ClientStatusC2SPacket.Mode.REQUEST_STATS));
        ...
        return Value.number(stat != null ? SYSHax.mc.player.getStatHandler().getStat(stat) : 0);
    }
    ...
    private static Value getItem(Starscript ss, int argCount) {
        ...
        return SYSHax.mc.player != null ? wrap(SYSHax.mc.player.getInventory().getStack(i)) : Value.null_();
    }
    ...
    private static Value countItems(Starscript ss, int argCount) {
        ...
        if (item == Items.AIR || SYSHax.mc.player == null) return Value.number(0);

        int count = 0;
        for (int i = 0; i < SYSHax.mc.player.getInventory().size(); i++) {
            ItemStack itemStack = SYSHax.mc.player.getInventory().getStack(i);
    ...
    private static Value oppositeX(boolean camera) {
        double x = camera ? SYSHax.mc.gameRenderer.getCamera().getPos().x : (SYSHax.mc.player != null ? SYSHax.mc.player.getX() : 0);
    ...
    private static Value oppositeZ(boolean camera) {
        double z = camera ? SYSHax.mc.gameRenderer.getCamera().getPos().z : (SYSHax.mc.player != null ? SYSHax.mc.player.getZ() : 0);
        Dimension dimension = PlayerUtils.getDimension();
    ...
    private static Value yaw(boolean camera) {
        float yaw;
        if (camera) yaw = SYSHax.mc.gameRenderer.getCamera().getYaw();
        else yaw = SYSHax.mc.player != null ? SYSHax.mc.player.getYaw() : 0;
        yaw %= 360;
    ...
    private static Value pitch(boolean camera) {
        float pitch;
        if (camera) pitch = SYSHax.mc.gameRenderer.getCamera().getPitch();
        else pitch = SYSHax.mc.player != null ? SYSHax.mc.player.getPitch() : 0;
        pitch %= 360;
    ...
    private static Value biome() {
        if (SYSHax.mc.player == null || SYSHax.mc.world == null) return Value.string("");

        BP.set(SYSHax.mc.player.getX(), SYSHax.mc.player.getY(), SYSHax.mc.player.getZ());
        Identifier id = SYSHax.mc.world.getRegistryManager().get(Registry.BIOME_KEY).getId(SYSHax.mc.world.getBiome(BP).value());
    ...
    private static Value handOrOffhand() {
        ...
        if (itemStack.isEmpty()) itemStack = SYSHax.mc.player.getOffHandStack();

        return itemStack != null ? wrap(itemStack) : Value.null_();
    }

    private static Value ping() {
        if (SYSHax.mc.getNetworkHandler() == null || SYSHax.mc.player == null) return Value.number(0);
    ...
    private static Value baritoneDistanceToGoal() {
        ...
        return Value.number((goal != null && SYSHax.mc.player != null) ? goal.heuristic(SYSHax.mc.player.getBlockPos()) : 0);
    }

    private static Value posString(boolean opposite, boolean camera) {
        Vec3d pos;
        if (camera) pos = SYSHax.mc.gameRenderer.getCamera().getPos();
        else pos = SYSHax.mc.player != null ? SYSHax.mc.player.getPos() : Vec3d.ZERO;
    ...
    private static Value crosshairType() {
        if (SYSHax.mc.crosshairTarget == null) return Value.string("miss");

        return Value.string(switch (SYSHax.mc.crosshairTarget.getType()) {
            case MISS -> "miss";
            case BLOCK -> "block";
            case ENTITY -> "entity";
        });
    ...
    private static Value crosshairValue() {
        if (SYSHax.mc.world == null || SYSHax.mc.crosshairTarget == null) return Value.null_();

        if (SYSHax.mc.crosshairTarget.getType() == HitResult.Type.MISS) return Value.string("");
        if (SYSHax.mc.crosshairTarget instanceof BlockHitResult hit) return wrap(hit.getBlockPos(), SYSHax.mc.world.getBlockState(hit.getBlockPos()));
        return wrap(((EntityHitResult) SYSHax.mc.crosshairTarget).getEntity());
    }
