...
package vince.syshax.utils.entity;

import vince.syshax.SYSHax;
import vince.syshax.mixin.CrossbowItemAccessor;
import vince.syshax.mixin.ProjectileInGroundAccessor;
import vince.syshax.mixininterface.IVec3d;
import vince.syshax.utils.Utils;
import vince.syshax.utils.misc.MissHitResult;
...
public class ProjectileEntitySimulator {
    ...
    public boolean set(Entity user, ItemStack itemStack, double simulated, boolean accurate, double tickDelta) {
        ...
        if (item instanceof BowItem) {
            double charge = BowItem.getPullProgress(SYSHax.mc.player.getItemUseTime());
    ...
    public HitResult tick() {
        ...
        if (pos.y < SYSHax.mc.world.getBottomY()) return MissHitResult.INSTANCE;

        // Check if chunk is loaded
        int chunkX = (int) (pos.x / 16);
        int chunkZ = (int) (pos.z / 16);
        if (!SYSHax.mc.world.getChunkManager().isChunkLoaded(chunkX, chunkZ)) return MissHitResult.INSTANCE;
    ...
    private boolean isTouchingWater() {
        ...
        FluidState fluidState = SYSHax.mc.world.getFluidState(blockPos);
    ...
    private HitResult getCollision() {
        ...
        HitResult hitResult = SYSHax.mc.world.raycast(new RaycastContext(vec3d3, pos3d, RaycastContext.ShapeType.COLLIDER, waterDrag == 0 ? RaycastContext.FluidHandling.ANY : RaycastContext.FluidHandling.NONE, SYSHax.mc.player));
        if (hitResult.getType() != HitResult.Type.MISS) {
            vec3d3 = hitResult.getPos();
        }

        HitResult hitResult2 = ProjectileUtil.getEntityCollision(SYSHax.mc.world, SYSHax.mc.player, vec3d3, pos3d, new Box(pos.x, pos.y, pos.z, pos.x, pos.y, pos.z).stretch(SYSHax.mc.player.getVelocity()).expand(1.0D), entity -> !entity.isSpectator() && entity.isAlive() && entity.canHit());
        if (hitResult2 != null) {
