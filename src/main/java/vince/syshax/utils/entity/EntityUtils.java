...
import vince.syshax.mixin.EntityTrackingSectionAccessor;
import vince.syshax.mixin.SectionedEntityCacheAccessor;
import vince.syshax.mixin.SimpleEntityLookupAccessor;
import vince.syshax.mixin.WorldAccessor;
...
public class EntityUtils {
    ...
    public static int getPing(PlayerEntity player) {
        ...
        PlayerListEntry playerListEntry = SYSHax.mc.getNetworkHandler().getPlayerListEntry(player.getUuid());
    ...
    public static boolean isAboveWater(Entity entity) {
        ...
        for (int i = 0; i < 64; i++) {
            BlockState state = SYSHax.mc.world.getBlockState(blockPos);
    ...
    public static boolean isInRenderDistance(double posX, double posZ) {
        double x = Math.abs(SYSHax.mc.gameRenderer.getCamera().getPos().x - posX);
        double z = Math.abs(SYSHax.mc.gameRenderer.getCamera().getPos().z - posZ);
        double d = (SYSHax.mc.options.getViewDistance().getValue() + 1) * 16;
    ...
    public static List<BlockPos> getSurroundBlocks(PlayerEntity player) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (SYSHax.mc.world.getBlockState(pos).getBlock() == Blocks.OBSIDIAN) {
                positions.add(pos);
            }
        }
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
        EntityLookup<Entity> entityLookup = ((WorldAccessor) SYSHax.mc.world).getEntityLookup();
