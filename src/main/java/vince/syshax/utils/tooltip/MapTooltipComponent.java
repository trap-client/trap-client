...
package vince.syshax.utils.tooltip;
...
public class MapTooltipComponent implements TooltipComponent, MeteorTooltipData {
    ...
    public void drawItems(TextRenderer textRenderer, int x, int y, MatrixStack matrices, ItemRenderer itemRenderer, int z) {
        ...
        // Contents
        VertexConsumerProvider.Immediate consumer = SYSHax.mc.getBufferBuilders().getEntityVertexConsumers();
        MapState mapState = FilledMapItem.getMapState(this.mapId, SYSHax.mc.world);
        if (mapState == null) return;
        matrices.push();
        matrices.translate(x, y, z);
        matrices.scale((float) scale, (float) scale, 0);
        matrices.translate(8, 8, 0);
        SYSHax.mc.gameRenderer.getMapRenderer().draw(matrices, consumer, this.mapId, mapState, false, 0xF000F0);
        consumer.draw();
        matrices.pop();
    }
}
