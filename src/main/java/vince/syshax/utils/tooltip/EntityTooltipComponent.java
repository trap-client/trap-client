...
public class EntityTooltipComponent implements MeteorTooltipData, TooltipComponent {
    ...
    protected void renderEntity(MatrixStack matrices, int x, int y) {
        if (SYSHax.mc.player == null) return;
        float size = 24;
        ...
        setupAngles();
        EntityRenderDispatcher entityRenderDispatcher = SYSHax.mc.getEntityRenderDispatcher();
        quaternion2.conjugate();
        entityRenderDispatcher.setRotation(quaternion2);
        entityRenderDispatcher.setRenderShadows(false);
        VertexConsumerProvider.Immediate immediate = SYSHax.mc.getBufferBuilders().getEntityVertexConsumers();
        entity.age = SYSHax.mc.player.age;
