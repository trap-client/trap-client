...
import vince.syshax.mixininterface.IExplosion;
import vince.syshax.mixininterface.IRaycastContext;
...
public class DamageUtils {
    ...
    public static void init() {
        SYSHax.EVENT_BUS.subscribe(DamageUtils.class);
    }

    @EventHandler
    private static void onGameJoined(GameJoinedEvent event) {
        explosion = new Explosion(SYSHax.mc.world, null, 0, 0, 0, 6, false, Explosion.DestructionType.DESTROY);
        raycastContext = new RaycastContext(null, null, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.ANY, SYSHax.mc.player);
    }
    ...
    public static double anchorDamage(LivingEntity player, Vec3d anchor) {
        SYSHax.mc.world.removeBlock(new BlockPos(anchor), false);
        double damage = bedDamage(player, anchor);
        SYSHax.mc.world.setBlockState(new BlockPos(anchor), Blocks.RESPAWN_ANCHOR.getDefaultState());
        return damage;
    }
    ...
    private static double getDamageForDifficulty(double damage) {
        return switch (SYSHax.mc.world.getDifficulty()) {
            case PEACEFUL -> 0;
    ...
    private static BlockHitResult raycast(RaycastContext context, BlockPos obsidianPos, boolean ignoreTerrain) {
        return BlockView.raycast(context.getStart(), context.getEnd(), context, (raycastContext, blockPos) -> {
            ...
            if (blockPos.equals(obsidianPos)) blockState = Blocks.OBSIDIAN.getDefaultState();
                ...
                blockState = SYSHax.mc.world.getBlockState(blockPos);
            ...
            VoxelShape voxelShape = raycastContext.getBlockShape(blockState, SYSHax.mc.world, blockPos);
            BlockHitResult blockHitResult = SYSHax.mc.world.raycastBlock(vec3d, vec3d2, blockPos, voxelShape, blockState);
