...
import vince.syshax.mixin.CreativeInventoryScreenAccessor;
import vince.syshax.mixin.HorseScreenHandlerAccessor;
...
public class SlotUtils {
    ...
    public static int indexToId(int i) {
        if (SYSHax.mc.player == null) return -1;
        ScreenHandler handler = SYSHax.mc.player.currentScreenHandler;
    ...
    private static int creativeInventory(int i) {
        if (!(SYSHax.mc.currentScreen instanceof CreativeInventoryScreen) || CreativeInventoryScreenAccessor.getSelectedTab() != ItemGroup.INVENTORY.getIndex()) return -1;
