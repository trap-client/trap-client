...
import vince.syshax.mixininterface.IChatHud;
...
public class ChatUtils {
    ...
    public static void init() {
        PREFIX = Text.literal("")
            ...
            .append("[")
            .append(Text.literal("Trap").setStyle(Style.EMPTY.withColor(new TextColor(SYSHax.ADDON.color.getPacked()))))
            .append("] ");
    }
    ...
    public static void sendPlayerMsg(String message) {
        SYSHax.mc.inGameHud.getChatHud().addToMessageHistory(message);

        if (message.startsWith("/")) SYSHax.mc.player.sendCommand(message.substring(1), null);
        else SYSHax.mc.player.sendChatMessage(message, null);
    }
    ...
    public static void sendMsg(int id, @Nullable String prefixTitle, @Nullable Formatting prefixColor, Text msg) {
        ...
        ((IChatHud) SYSHax.mc.inGameHud.getChatHud()).add(message, id);
    }
