...
import vince.syshax.mixininterface.IMatrix4f;
import vince.syshax.utils.Utils;
import vince.syshax.utils.misc.Vec3;
import vince.syshax.utils.misc.Vec4;
...
public class NametagUtils {
    ...
    public static void onRender(MatrixStack matrices, Matrix4f projection) {
        ...
        camera.set(SYSHax.mc.gameRenderer.getCamera().getPos());
        cameraNegated.set(camera);
        cameraNegated.negate();

        windowScale = SYSHax.mc.getWindow().calculateScaleFactor(1, false);
    }
    ...
    public static boolean to2D(Vec3 pos, double scale) {
        ...
        pmMat4.toScreen();
        double x = pmMat4.x * SYSHax.mc.getWindow().getFramebufferWidth();
        double y = pmMat4.y * SYSHax.mc.getWindow().getFramebufferHeight();

        if (Double.isInfinite(x) || Double.isInfinite(y)) return false;

        pos.set(x / windowScale, SYSHax.mc.getWindow().getFramebufferHeight() - y / windowScale, pmMat4.z);
        return true;
    }
