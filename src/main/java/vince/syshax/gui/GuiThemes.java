...
import vince.syshax.gui.themes.meteor.MeteorGuiTheme;
...
public class GuiThemes {
    private static final File FOLDER = new File(SYSHax.FOLDER, "gui");
    ...
    public static void init() {
        ...
        add(new TrapGuiTheme());
        add(new SYSHaxGuiTheme());
    }
    ...
    public static void add(GuiTheme theme) {
        for (Iterator<GuiTheme> it = themes.iterator(); it.hasNext();) {
            if (it.next().name.equals(theme.name)) {
                ...
                SYSHax.LOG.error("Theme with the name '{}' has already been added.", theme.name);
                break;
            }
        }
